...
const previewPays = [
    {
        label: 'Pay #14',
        date: '12 Jul',
        bills: ['Car loan', 'Internet'],
        surplus: '$1,320',
    },
    {
        label: 'Pay #15',
        date: '26 Jul',
        bills: ['Mortgage', 'Mobile'],
        surplus: '$900',
    },
    {
        label: 'Card cycle',
        date: '4 Aug',
        bills: ['Credit card statement'],
        surplus: '$1,980',
    },
];
...
export default function Welcome() {
    ...
    return (
        <div
            className={`min-h-screen ${isDark ? 'bg-[#05060b] text-white' : 'bg-[#f8fafc] text-slate-900'}`}
        >
            <Head title="PlanYourPay" />
            <div className="mx-auto flex max-w-5xl flex-col gap-12 px-6 py-10">
                <header
                    className={`flex flex-col gap-4 text-sm ${isDark ? 'text-white/70' : 'text-slate-600'} sm:flex-row sm:items-center sm:justify-between`}
                >
                    <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            <span
                                className={`inline-flex h-10 w-10 items-center justify-center rounded-full text-base font-semibold ${isDark ? 'bg-white text-slate-900' : 'bg-slate-900 text-white'}`}
                            >
                                PY
                            </span>
                            <p
                    ...
                    <nav className="flex flex-wrap gap-3">
                        {auth.user ? (
                            <Link
                                href={dashboard()}
                                className={`rounded-full border px-4 py-1.5 ${isDark ? 'border-white/20' : 'border-slate-900/20 text-slate-700'}`}
                            >
                                Dashboard
                            </Link>
                        ) : (
                ...
                <section className="grid gap-10 md:grid-cols-2">
                    <div className="space-y-6">
                        <h1 className="text-4xl leading-tight font-semibold md:text-5xl">
                            Your pay, already assigned.
                        </h1>
                        <p
                            className={`text-sm ${isDark ? 'text-white/70' : 'text-slate-600'}`}
                        >
                            PlanYourPay keeps weekly, fortnightly, or monthly
                            pays in sync with every bill.
                        </p>
                    ...
                    <div
                        className={`rounded-3xl p-5 text-sm ${isDark ? 'bg-white/5' : 'bg-white shadow-[0_20px_40px_rgba(15,23,42,0.08)]'}`}
                    >
                        ...
                        <div className="mt-5 space-y-3">
                            <div className="space-y-2 border-t border-white/10 pt-3">
                                ...
                                <ul
                                    className={`space-y-1 ${isDark ? 'text-white/70' : 'text-slate-600'}`}
                                >
                                ...
                                <div className="flex items-center justify-between text-sm">
                                    ...
                                    <span
                                        className={`text-xl font-semibold ${isDark ? 'text-emerald-300' : 'text-emerald-600'}`}
                                    >
                            ...
                            >
                                Tap a tab to experience a real pay brief—no
                                login required.
                            </p>
                        </div>
                    </div>
                ...
                <section
                    className={`space-y-3 rounded-3xl p-6 ${isDark ? 'bg-white/5' : 'bg-white shadow-[0_30px_60px_rgba(15,23,42,0.05)]'}`}
                >
                    {features.map((feature) => (
                        <div
                            key={feature}
                            className="flex items-center gap-3 border-b border-white/10 pb-3 text-sm last:border-b-0"
                        >
                            <span
                                className={`text-xs font-semibold ${isDark ? 'text-white/40' : 'text-slate-500'}`}
                            >
                                •
                            </span>
                            <span
                ...
                <section className="space-y-4">
                    <p
                        className={`text-xs tracking-[0.4em] uppercase ${isDark ? 'text-white/50' : 'text-slate-500'}`}
                    >
                        Flow
                    </p>
                ...
                <section className="text-center">
                    ...
                    <p
                        className={`mt-3 text-sm ${isDark ? 'text-white/70' : 'text-slate-600'}`}
                    >
                ...
                <footer
                    className={`pb-6 text-center text-xs ${isDark ? 'text-white/50' : 'text-slate-500'}`}
                >
